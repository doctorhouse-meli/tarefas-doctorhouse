...
test('som é compartilhado no banco e apenas Admin pode salvar', async () => {
  ...
  for (const sound of ['meme-faaah','meme-aura','meme-pix','meme-magnata','meme-bruh']) {
    stored = { sound, volume: 85, repeat: 2 };
    const migrated = await vm.runInContext('getNotificationSettings()', context);
    assert.equal(migrated.sound, 'original');
    assert.equal(migrated.volume, 85);
    assert.equal(migrated.repeat, 2);
    assert.throws(() => validateNotificationSettings(stored));
  }
