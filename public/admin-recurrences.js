...
function renderAdminRecurrences() {
  ...
  document.querySelector('#recurrenceList').innerHTML = visible.length ? visible.map(template => {
    ...
    return `<article class="recurrence-card">
      ...
      <div class="recurrence-actions"><button type="button" class="btn-secondary" ${hasTaskPermission('editarTarefas') ? '' : 'hidden'} data-recurrence-action="edit" data-id="${escapeHtml(template.id)}">Editar</button><button type="button" class="btn-secondary" ${hasTaskPermission('editarTarefas') && hasTaskPermission('criarParaOutros') ? '' : 'hidden'} data-recurrence-action="assign" data-id="${escapeHtml(template.id)}">Trocar responsável</button><button type="button" class="text-button recurrence-delete" ${hasTaskPermission('excluirTarefas') ? '' : 'hidden'} data-recurrence-action="delete" data-id="${escapeHtml(template.id)}">Excluir</button></div>
    </article>`;
